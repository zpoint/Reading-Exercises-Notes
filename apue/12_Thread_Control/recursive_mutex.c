...
#define clock_nqnosleep(ID, FL, REQ, REM) nanosleep((REQ), (REM))
#endif

#ifndef CLOCK_REALTIME
#define CLOCK_REALTIME 0
#define USETONSEC 1000 // microseconds to nanoseconds

void clock_gettime(int id, strct timespec *tsp)
{
		struct timeval tv;	
...
void *timeout(const struct timespec *when, void (*func)(void *), void *arg)
{
		...
		// get here if when <= now, or malloc fails, or we can't make a thread, so just call the function now
		(*func)(arg);
}
