...
int main(int argc, char *argv[])
{
		...
		char *addrStrinfg = argv[1]; // Server address/name
		...
		// Get address(es) associated with specificed name/service
		struct addrinfo *addrList; // Holder for list of addresses retuened
		// Modify servAddr contents to reference linked list of addresses
		int rtnVal = getaddrinfo(addrStrinfg, portString, &addrCriteria, &addrList);
		if (rtnVal != 0)
