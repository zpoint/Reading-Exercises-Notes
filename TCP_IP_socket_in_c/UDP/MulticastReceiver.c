...
int main(int argc, char *argv[])
{
		...
		char *multicastAddrString = argv[1]; // First arg: multicasr addr (v4 or v6)
		char *service = argv[2]; // Second arg: post/service
		...
		else if (multicastAddr->ai_family == AF_INET)
		{
				...
				joinRequest.imr_interface.s_addr = 0; // Let system choose the i/f
		...
		char recvString[BUFSIZ + 1]; // Buffer to receive into
		// Receive a single datagram from the server
		int recvStringLen = recvfrom(sock, recvString, BUFSIZ, 0, NULL, 0);
		if (recvStringLen < 0)
