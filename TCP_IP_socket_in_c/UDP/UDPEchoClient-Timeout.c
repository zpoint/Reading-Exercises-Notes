...
static const size_t MAXSTRINGLENGTH = BUFSIZ;

unsigned int tries = 0; // Count of time sent - GLOBAL for signal-handler access
...
int main(int argc, char *argv[])
{
		...
		while ((numBytes = recvfrom(sock, buffer, MAXSTRINGLENGTH, 0, (struct sockaddr *)&fromAddr, &fromAddrLen)) < 0)
		{
				if (errno == EINTR)
				{
						...
						if (tries < MAXTRIES)
						{
								printf("timeout after %d seconds, %d more times to try...\n", TIMEOUT_SECS, MAXTRIES-tries);
								// Incremented by signal handler
								numBytes = sendto(sock, echoString, echoStringLen, 0, servAddr->ai_addr, servAddr->ai_addrlen);
								if (numBytes < 0)
										DieWithSystemMessage("sento() failed");
								else if (numBytes != echoStringLen)
										DieWithUserMessage("sendto() error", "sent unexpected number of bytes");
								alarm(TIMEOUT_SECS);
						}
		...
		buffer[echoStringLen] = '\0'; // Null-termiinate the received data
