...
int main(int argc, char *argv[])
{
		...
		if (sendStringLen > BUFSIZ) // Check input length
		...
		struct addrinfo *multicastAddr; // Holder for retuened address
		...
		for (;;)
		{
				...
				ssize_t numBytes = sendto(sock, sendString, sendStringLen, 0, multicastAddr->ai_addr, multicastAddr->ai_addrlen);
				if (numBytes < 0)
