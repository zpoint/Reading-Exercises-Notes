...
int main(int argc, char *argv[])
{
		...
		long timeout = atol(argv[1]); // First arg: Timeot
		int noPorts = argc - 2; // Number of ports iis argument count minus 2
		...
		while (running)
		{
				...
				for (int port = 0; port < noPorts; port++)
				{
						...
						{
								printf("Request on port %d, socket num: %d: ", port, servSock[port]);
								HandleTCPClientSleep(AcceptTCPConnection(servSock[port]), 1);
						}
				}
		}
}
