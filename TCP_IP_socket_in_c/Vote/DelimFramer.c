...
/* Read up to buffSize bytes or until delimiter, copying into the given
 * buffer as we go
 * Encountering EOF after some data but before delimiter results in failure
 * (That is: EOF is not a valid delimiter. )
 * Retuens the number of bytes placed in buf (delimiter NOT transferred).
...
int GetNextMsg(FILE *in, uint8_t *buf, size_t buffSize)
{
		...
		while (count < buffSize)
		{
				...
				{
						if (count > 0)
								DieWithUserMessage("GetNextMsg()", "Stream ended prematrely");
						else
								return -1;
				}
