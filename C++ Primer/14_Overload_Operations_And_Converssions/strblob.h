...
class StrBlobPtr
{
		...
		friend StrBlobPtr operator+(StrBlobPtr&, std::size_t);
		friend StrBlobPtr operator-(StrBlobPtr&, std::size_t);
		public:
				StrBlobPtr(): curr(0) { }
...
std::ostream& operator<<(std::ostream &os, const StrBlobPtr &ptr)
{
		ptr.check(ptr.curr, "operator<< check one past end of StrBlobPtr");
		os << ptr.deref();
		return os;
}

// 14_28.cpp
StrBlobPtr operator+(StrBlobPtr &ptr, std::size_t size)
{
		StrBlobPtr new_ptr(ptr);
		new_ptr.curr += size;
		new_ptr.check(new_ptr.curr, "operator+ past end of StrBlobPtr");
		return new_ptr;
}

StrBlobPtr operator+(std::size_t size, StrBlobPtr &ptr)
{
		return ptr + size;
}
...
StrBlobPtr operator-(StrBlobPtr &ptr, std::size_t size)
{
		StrBlobPtr new_ptr(ptr);
		new_ptr.curr -= size;
		new_ptr.check(new_ptr.curr, "operator- past begin of StrBlobPtr");
		return new_ptr;
}
...
{
		auto p = check(curr, "defrence past end");
		return (*p)[curr];
}
