...
class Token
{
		public:
			...
			Token(const Token&&); // move constructor
			Token(const Token &t): tok(t.tok) { copyUnion(t); }
			Token &operator=(const Token&);
			Token &operator=(const Token&&); // move assignment
...
void Token::moveUnion(const Token &t)
{
		switch (t.tok)
		{
				...
				case Token::SDT: sdt_val = std::move(t.sdt_val); break;
		}
}

Token &Token::operator=(const Token &t)
{
		std::cout << "this->tok: " << this->tok << std::endl;
		std::cout << "rhs.tok: " << t.tok << std::endl;
