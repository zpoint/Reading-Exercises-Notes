...
class StrVec {
		public:
				...
				~StrVec();
				StrVec(StrVec &&rhs): elements(rhs.elements), first_free(rhs.first_free), cap(rhs.cap) { std::cout << "Move constructor" << std::endl; rhs.elements = rhs.first_free = rhs.cap = nullptr; }
...
int main()
{
		...
		StrVec ivec2(std::move(ivec));
		for (const auto &str : ivec2)
				std::cout << str << " ";
		std::cout << std::endl;
		return 0;
}
