...
#define NBUFF 10
#define SEM_MUTEX "mutex" /* there are args to px_ipc_name() */
...
int main(int argc, char **argv)
{
		...
		shared.mutex = Sem_open(Px_ipc_name(SEM_MUTEX), O_CREAT | O_EXCL, FILE_MODE, NBUFF);
		...
		/* create one producer thread and one comsumer thread */
		Set_concurrency(2);
...
void *produce(void *arg)
{
		...
		for (i = 0; i < nitems; i++)
		{
				...
				Sem_post(shared.nstored); /* i more stored item */
		}
		return (NULL);
}
